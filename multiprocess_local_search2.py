...
def local_search(SOURCE_PATH,point,m, return_list,pas):
    neighborhood = [(pas,0),(-pas,0),(0,pas),(0,-pas)]
    current_min = point
...
def batch_local_search(SOURCE_PATH, points, m_heur,pas):
    manager = mp.Manager()
    return_list = manager.list()
    processes = []
    for point in points:
        print(f'starting local search for {point}')
        p = mp.Process(target=local_search, args=(SOURCE_PATH, point, m_heur, return_list,pas))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
