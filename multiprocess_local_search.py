...
### fonction de local_search avec variation de la masse de TNT

neighborhood_tnt = [(pas_search,0,0),(-pas_search,0,0),(0,pas_search,0),(0,-pas_search,0),(0,0,pas_tnt),(0,0,-pas_tnt)] # croix de distance 1 

def local_search_with_tnt(SOURCE_PATH,point,m, return_list):
    current_min = point + [m]
    current_mincost = np.inf
    print(f'starting local search for {point}')
    ...
    while iteration < max_iter_space and  (current_min[1]-point[1])**2 + (current_min[0]-point[0])**2 < pas_gs**2:
        batch = []   #Construction du batch de simulation pour l'étape
        for ng in neighborhood_tnt:
            next_point = (current_min[0]+ng[0],current_min[1]+ng[1],current_min[2]+ng[2])
            if bati.test_point_inside(next_point[0],next_point[1])==0:
                batch.append({'XC':next_point[0],'YC':next_point[1],'M_TNT':next_point[2],'PAS':pas_sim})
        SIM_PATHS = launch.batch_launch_simulation(batch) #liste des paths résultats des simus
        ...
        for path in SIM_PATHS:  #recherche du meilleur point
            c = costf.cost_function1(SOURCE_PATH,path+'/POST1D/TE')
            p = (float(path.split('_')[5]),float(path.split('_')[6],float(path.split('_')[7]))
            next_c.append(c)
            next_p.append(p)
        index = np.argmin(np.array(next_c))
