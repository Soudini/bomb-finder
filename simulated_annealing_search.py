...
def simulated_annealing_search(SOURCE_PATH,point,m):     # Algorithme du recuit simulé

    current_best      = point
    batch             = []
    if bati.test_point_inside(point[0], point[1]) == 0:
        batch.append({'XC': point[0], 'YC': point[1], 'M_TNT': m, 'PAS': pas_neighb})
    SIM_PATHS = launch.batch_launch_simulation(batch)      # Simulation du point de départ
    ...
    while k<k_max:
        ...
        SIM_PATHS = launch.batch_launch_simulation(batch)      # Simulation d'un point du voisinage
        new_cost = costf.cost_function1(SOURCE_PATH, SIM_PATHS[0] + '/POST1D/TE')

        if new_cost < current_cost or random.random() < np.exp(-(new_cost-current_cost)/T):      # Sélection du point selon le seuil de température
        ...
        T = T_0*(1-k/k_max)     # Evolution linéaire de la température
        k = k+1
